package com.kh.variable;

public class Variable {

	//변수 : Memory(RAM)에 값을 저장하기 위한 공간
	
	// 변수 그냥 한번 써보기 --> 설명을 위해서 구현하는 것이니 실제로는 이렇게 하지 마세요.
	public void 바지를저장하는기능() {
		String 옷장 = "긴바지";
		String 행거 = "반바지";
		System.out.println(옷장);
	}

	//점심메뉴 출력 프로그램(키오스크)
	public void findLunchMenu() {
	
		/* 1. 메뉴들을 출력(메뉴명, 가격)
		 * 2. 메뉴판을 보고 주문을 했다 가정
		 * 3. 내야할 금액을 출력
		 * 
		 */
		System.out.println("---변수를 사용하기 전 ---");
		System.out.println("메뉴판 >");
		System.out.println("1. 트리플 치즈버거 세트(15000)");
		System.out.println("2. 빅맥 세트(8397)");
		System.out.println("3. 슈슈버거 세트(9800");
		
	
		System.out.println("트리플치즈버거 4개 주문");
		System.out.println(15000 * 4 + "원입니다");
		System.out.println("빅맥 9개 주문");
		System.out.println(8397 * 9 + "원입니다.");
		System.out.println("슈슈버거 4개 주문");
		System.out.println(9800 * 4 +"원입니다.");
		System.out.println("------");
		
		System.out.println("변수 사용해보기");
		//메뉴명과 가격이라는 값을 담을 곤간
		//변수를 선언하고 변수공간에 값을 대입해서 사용해보기
		String setOne = "KH 아카데미표 슈슈버거 세트";
		int setOnePrice = 9800;
		String setTwo = "KH 아카데미표 비크매크 세트";
		int setTwoPrice = 8800;
		
		System.out.println("메뉴판 > ");
		System.out.println(setOne + ", 가격 : " + setOnePrice);
		System.out.println(setTwo + ", 가격 : " + setTwoPrice);

		System.out.println(setOne + "를 10개 시켰습니다.");
		System.out.println((setOnePrice * 10) + "원입니다.");
		System.out.println(setTwo + "를 5개 시켰습니다.");
		System.out.println((setTwoPrice * 5) + "원입니다.");
	}

		//변수 사용방법 알아보기
		public void declareVariable() {
			int price; //변수 선언
			boolean isTure = true;
			System.out.println(isTure);
			isTure = false;
			System.out.println(false);
		
		
		byte byteNumber = 1;
		short shortNumber = 2;
		int intNumber = 3;
		long longNumber = 4L;
		
		float floatNumber = 2.22F;
		double doubleNumber = 3.33;
		char money = '돈';
		String str = "문자열";
		System.out.println("정수 자료형 :" + byteNumber);
		System.out.println("정수 자료형 :" + shortNumber);
		System.out.println("정수 자료형 :" + intNumber);
		System.out.println("정수 자료형 :" + longNumber);
		System.out.println("실수 자료형 :" + floatNumber);
		System.out.println("실수 자료형 :" + doubleNumber);
		System.out.println("문자 자료형 :" + money);
		System.out.println("문자 자료형 :" + str);
		
		System.out.printf("논리 자료형 :%b, 정수 자료형 : %d \n", isTure, intNumber);
		System.out.printf("doubleNum 값 : %.1f",doubleNumber);
		
		final int startYear = 1998;
		
		}
}

	